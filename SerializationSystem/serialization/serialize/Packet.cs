...
namespace SerializationSystem.Internal {
    ...
    internal class Packet {
        ...
        internal void Write(Type type, object obj, SerializeMode serializeMode, ModelType modelType) {
            ...
            if (SerializeUtils.BuiltinTypes.Contains(type)) WriteBuiltin(type, obj, modelType);
            else if (type.IsEnum) WriteEnum(type, obj, modelType);
            else if (type == typeof(Type)) WriteTypeId(((Type) obj).ID(), modelType);
            else WriteNullable(type, obj, serializeMode, modelType);
        }

        private void WriteBuiltin(Type type, object obj, ModelType modelType) {
            ...
            else if (type == typeof(string)) WriteNullable(type, obj, SerializeMode.Default, modelType);
        }

        private void WriteNullable(Type type, object obj, SerializeMode serializeMode, ModelType modelType) {
            ...
            else if (SerializeUtils.CanSerializeList(type)) WriteList(type, obj, serializeMode, modelType);
            else if (SerializeUtils.CanSerializeDictionary(type)) WriteDictionary(type, obj, serializeMode, modelType);
            else Serializer.Serialize(obj, type, this, serializeMode, modelType);
        }

        internal void Write<T>(T obj, SerializeMode serializeMode, ModelType modelType) {
            var type = typeof(object);
            if (obj != null) type = obj.GetType();
            Write(type, obj, serializeMode, modelType);
        }
        ...
        private void WriteList(Type type, object obj, SerializeMode serializeMode, ModelType modelType) {
            var list = (IList) obj;
            var elemType = SerializeUtils.GetListElementType(type);
            Write(list.Count, SerializeMode.Default, modelType);
            foreach (var elem in list) {
                Write(elemType, elem, serializeMode, modelType);
            }
        }
        ...
        private void WriteDictionary(Type type, object obj, SerializeMode serializeMode, ModelType modelType) {
            ...
            WriteList(typeof(List<>).MakeGenericType(keyType), keyList, serializeMode, modelType);
            WriteList(typeof(List<>).MakeGenericType(valueType), valueList, serializeMode, modelType);
        }
        ...
        private void WriteEnum(Type type, object obj, ModelType modelType) {
            var underlyingType = type.GetEnumUnderlyingType();
            var underlyingValue = Convert.ChangeType(obj, underlyingType);
            Write(underlyingType, underlyingValue, SerializeMode.Default, modelType);
        }
        ...
        internal void WriteTypeId(TypeId typeId, ModelType modelType) {
            ...
            if (!type.IsGenericType) {
                ...
                Write(typeId.ID, SerializeMode.Default, modelType);
                return;
            }
            ...
            Write(typeDef.ID, SerializeMode.Default, modelType);
            var genericArguments = type.GetGenericArguments();
            foreach (var arg in genericArguments) {
                WriteTypeId(TypeIdUtils.Get(arg), modelType);
            }
        }
